...
export default function OperationCard({ operation }) {
  ...
  return (
    <div className="bg-pf-surface rounded-xl border border-pf-border p-5 hover:border-pf-cyan/20 transition-all duration-300">
      <div className="flex items-start gap-3 mb-3">
        <div className="p-1.5 rounded-lg bg-pf-red/10 mt-0.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            >
              {status.pulse && <span className="w-1.5 h-1.5 rounded-full status-dot" style={{ background: status.color }} />}
              {status.label}
            </span>
          </div>
      ...
      <div className="flex items-center gap-4 mt-4 pt-3 border-t border-pf-border/50">
        ...
        <div className="ml-auto">
          <button className="px-3 py-1.5 text-xs font-semibold text-pf-cyan bg-pf-cyan/10 hover:bg-pf-cyan/20 border border-pf-cyan/20 rounded-lg transition-colors">
            View Details →
          </button>
        </div>
      </div>
